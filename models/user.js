...
UserSchema.methods.Buildings = async function(){
    let err, buildings;
    [err, buildings] = await to(Building.find({'users.user':this._id}));
    if(err) TE('err getting buildings');
    return buildings;
};

UserSchema.methods.Users = async function(){
    let err, users;
    [err, users] = await to(User.findAll());
    if(err) TE('err getting users');
    return users;
};
